...
for (let i=0; i<=8; i++) {
  ...
  const towards = camera.position.clone().sub(mesh.position).normalize();
...
manager.update(.1);
assert.ok(Math.abs(states.at(-1).distance-2)<.25);
...
assert.deepEqual(creature.anchorPosition.toArray(),[0,3.2,2]);
assert.ok(Math.abs(mesh.position.x)<=ammoniteSpecies.movement.ambientMotion.driftXAmplitude);
assert.ok(Math.abs(mesh.position.y-3.2)<=ammoniteSpecies.movement.ambientMotion.verticalAmplitude);
assert.ok(Math.abs(mesh.position.z-2)<=ammoniteSpecies.movement.ambientMotion.driftZAmplitude);
