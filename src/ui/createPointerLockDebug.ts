  ...
  return {
    ...
    setCreatureState(state: CreatureDebugState): void {
      directionInfo.textContent = [
        ...
        `Primary View: ${state.view ?? '—'}`,
        `Secondary View: ${state.secondaryView ?? 'NONE'}`,
        `Blend: ${(state.horizontalBlend ?? 0).toFixed(2)}`,
        `Blend Boundary: ${state.blendBoundary === undefined ? '—' : state.blendBoundary.toFixed(1) + '°'}`,
